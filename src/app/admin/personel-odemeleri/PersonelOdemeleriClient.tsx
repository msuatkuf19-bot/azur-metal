...
import { PAYMENT_METHOD_LABELS, WORKER_PAYMENT_TYPE_LABELS, WORKER_PAYMENT_TYPE_COLORS } from '@/lib/constants';
...
export default function PersonelOdemeleriClient({ data }: { data: any }) {
  const router = useRouter();
  const { payments, workers, jobs } = data;

  const [filters, setFilters] = useState({ workerId: '', paymentType: '', startDate: '', endDate: '' });
  const [drawer, setDrawer] = useState<{ open: boolean; editing: any | null }>({ open: false, editing: null });

  const hasActiveFilter = Object.values(filters).some((v) => v !== '');
  const clearFilters = () => setFilters({ workerId: '', paymentType: '', startDate: '', endDate: '' });
  ...
  const filtered = useMemo(() => {
    return payments.filter((p: any) => {
      ...
      if (filters.endDate && new Date(p.date) > new Date(filters.endDate + 'T23:59:59')) return false;
      return true;
    });
  }, [payments, filters]);

  const totalPaid = filtered.reduce((s: number, p: any) => s + p.amount, 0);
  const advanceTotal = filtered.filter((p: any) => p.paymentType === 'AVANS').reduce((s: number, p: any) => s + p.amount, 0);

  const [deleteTarget, setDeleteTarget] = useState<any | null>(null);

  const handleDelete = async () => {
    if (!deleteTarget) return;
    const r = await deleteWorkerPayment(deleteTarget.id);
    if (r.success) { toast.success((r as any).message || 'Silindi'); setDeleteTarget(null); router.refresh(); }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={`Tüm personele yapılan ödemeler • ${payments.length} kayıt`}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard title="Avans Toplamı" value={formatCurrency(advanceTotal)} accent="warning" />
        <StatCard title="Personel Sayısı" value={workers.length} accent="default" />
      </div>
        ...
        <CardBody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select value={filters.paymentType} onChange={(e) => setFilters({ ...filters, paymentType: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <Card>
        <CardBody className="p-0">
          {filtered.length === 0 ? (
            <EmptyState title={hasActiveFilter ? 'Filtrelere uygun kayıt bulunamadı' : 'Henüz personel ödemesi yok'} actionLabel={!hasActiveFilter ? 'İlk ödemeyi ekle' : undefined} onAction={!hasActiveFilter ? () => setDrawer({ open: true, editing: null }) : undefined} />
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {filtered.map((p: any) => (
                    <tr key={p.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2.5"><Link href={`/admin/tanimlamalar/ustalar/${p.workerId}`} className="text-blue-600 hover:underline font-medium">{p.workerName}</Link></td>
                      ...
                      <td className="px-4 py-2.5 text-slate-500 max-w-[200px] truncate">{p.description || '-'}</td>
      ...
      <WorkerPaymentDrawer state={drawer} onClose={() => setDrawer({ open: false, editing: null })} workers={workers} jobs={jobs} onSaved={() => { setDrawer({ open: false, editing: null }); router.refresh(); }} />
      ...
      <DeleteConfirmDialog
        ...
        description={deleteTarget ? `${deleteTarget.workerName} — ${formatDate(deleteTarget.date)} tarihli ${formatCurrency(deleteTarget.amount)} tutarındaki ödeme silinecek. Personel bakiyesi yeniden hesaplanır.` : ''}
      />
    </div>
  );
}

function WorkerPaymentDrawer({ state, onClose, workers, jobs, onSaved }: any) {
  ...
  if (open && lastKey !== formKey) {
    setLastKey(formKey);
    setForm({
      workerId: isEdit ? editing.workerId : (workers[0]?.id || ''),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isLoading) return;
    if (!form.workerId) { toast.error('Personel seçin'); return; }
    const amount = parseMoney(form.amount);
    if (isNaN(amount) || amount <= 0) { toast.error('Tutar sıfırdan büyük olmalı'); return; }

    setIsLoading(true);
    const payload = { workerId: form.workerId, amount, date: form.date, paymentType: form.paymentType, paymentMethod: form.paymentMethod, jobId: form.jobId || null, description: form.description || undefined };
  ...
  return (
    <Drawer isOpen={open} onClose={onClose} title={isEdit ? 'Ödeme Düzenle' : 'Ödeme Ekle'} size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Personel</label>
          <select value={form.workerId} onChange={(e) => setForm({ ...form, workerId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Ödeme Türü</label>
            <select value={form.paymentType} onChange={(e) => setForm({ ...form, paymentType: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
              {Object.entries(WORKER_PAYMENT_TYPE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </div>
        </div>
